...
def swr_end_extended_spikes_gain(basepath, sampling_rate=1250, extension_ms=200):
    """
    Calculates SWR spike gain while extending the end time of SWR events by a specified duration.
    
    Parameters:
        basepath (str): Path to the data files.
        sampling_rate (float): Sampling rate in Hz (default 1250 Hz).
        extension_ms (float): Extension in milliseconds to add to the end of the SWR duration.
    
    Returns:
        dict: Cell metrics with ripple gain, firing rates, and participation metrics.
    """
    st, cell_metrics = load_spikes(basepath)
    ripples = load_ripples_events(basepath)

    # Convert extension from ms to samples
    extension_samples = (extension_ms / 1000) * sampling_rate

    # Extend the stop times only
    ripples['stop'] = ripples['stop'] + extension_samples

    # Create EpochArray from modified ripple events
    ripple_epochs = nel.EpochArray([np.array([ripples['start'], ripples['stop']]).T])

    # Calculate average firing rates inside and outside ripple events
