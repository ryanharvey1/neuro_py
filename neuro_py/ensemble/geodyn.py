...
from ..util.array import find_terminal_masked_indices
...
def potential_landscape(X_dyn, projbins, domainbins=None):
    ...
    for nnrn in range(nnrns):
        ...
        H, bin_edges, _ = binned_statistic_dd(  # posbins x time
            np.asarray((X_t_flat[:, nnrn], norm_tpts)).T, delta_t_flat[:, nnrn], statistic='count',
            bins=(projbins if type(projbins) == int else projbins[nnrn], nbins_domain))
        latentedges, domainedges = bin_edges

        grad_pos_t_svm = binned_statistic_dd(
            np.asarray((X_t_flat[:, nnrn], norm_tpts)).T, delta_t_flat[:, nnrn], statistic='sum',
            bins=(projbins if type(projbins) == int else projbins[nnrn], nbins_domain)).statistic
        ...
        offset: np.ndarray[np.Any, np.dtype[np.Any]] = potential_pos_t[idx_zero_X_t, :]  # use potential at X_t = 0 as reference
    ...
    return (
        potential_pos_t_nrns, grad_pos_t_svm_nrns, hist, latentedges_nrns,
        domainedges_nrns,
    )

def potential_landscape_nd(X_dyn, projbins, domainbins=None):
    """Compute numerical approximation of potential energy landscape across
    state and domain.
    ...
    for nnrn in range(nnrns):
        ...
        H, bin_edges, _ = binned_statistic_dd(  # (nnrns times projbins) x time
            ...
            bins=(*[projbins if type(projbins) == int else projbins[idx] for idx in range(nnrns)], nbins_domain))
        latentedges= bin_edges[nnrn]
        domainedges = bin_edges[-1]

        grad_pos_t_svm = binned_statistic_dd(
            np.asarray((*X_t_flat.T, norm_tpts)).T, delta_t_flat[:, nnrn], statistic='sum',
            bins=(*[projbins if type(projbins) == int else projbins[idx] for idx in range(nnrns)], nbins_domain)).statistic
        # average derivative, a.k.a. flow/vector field for dynamics underlying
        # population activity
        grad_pos_t_svm = np.divide(grad_pos_t_svm, H, where=H != 0)
        grad_pos_t_svm[H == 0] = np.nan  # crucial to handle division by zero
        ...
        idx_terminal_nonzero = np.asarray(list(zip(*npy.util.find_terminal_masked_indices(
            nonzero_mask, axis=nnrn))))  # unzip the list
        print(idx_terminal_nonzero.shape)
        ...
        if nnrns == 1:
            ...
            for pb in range(projbins):  # extend for nnrns > 2
                ...
                for t in range(nbins_domain):
                    ...
                    if idx_first_nonzero == 0 and idx_last_nonzero == projbins - 1:
