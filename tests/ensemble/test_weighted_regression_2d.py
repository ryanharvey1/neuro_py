...
def test_weighted_regression_2d_jit_lazy_export():
    import neuro_py.ensemble as ensemble
    from neuro_py.ensemble import replay

    assert ensemble.weighted_regression_2d_jit is replay.weighted_regression_2d_jit


class TestWeightedRegression2D:
    """Test suite for weighted_regression_2d function."""
    ...
    def test_basic_functionality(self):
        ...
        result = weighted_regression_2d(weights, x_coords, y_coords, time_coords)

        # Check result structure
        assert len(result) == 7
        assert isinstance(result[0], float)  # r2
    ...
    def test_default_coords(self):
        ...
        result = weighted_regression_2d(weights)
    ...
    def test_different_dtypes(self):
        ...
        result_f32 = weighted_regression_2d(weights_f32)

        # Float64 test
        weights_f64 = np.array([[[1, 2], [3, 4]], [[5, 6], [7, 8]]], dtype=np.float64)
        result_f64 = weighted_regression_2d(weights_f64)
        ...
        assert_allclose(result_f32[0], result_f64[0], rtol=1e-6)  # r2
    ...
    def test_perfect_diagonal_and_anti_diagonal_r2(self):
        """Both diagonal directions have perfect unsigned sequence strength."""
        n = 3
        diagonal_weights = np.zeros((n, n, n), dtype=np.float64)
        for t in range(n):
            diagonal_weights[t, t, t] = 1.0

        diagonal_result = weighted_regression_2d(diagonal_weights)

        assert_allclose(diagonal_result[0], 1.0, atol=1e-12)
        assert_allclose(diagonal_result[3], 1.0, atol=1e-12)
        assert_allclose(diagonal_result[4], 1.0, atol=1e-12)

        anti_diagonal_weights = np.zeros((n, n, n), dtype=np.float64)
        for t in range(n):
            anti_diagonal_weights[t, n - 1 - t, t] = 1.0

        anti_diagonal_result = weighted_regression_2d(anti_diagonal_weights)

        # Opposing slopes no longer cancel the sequence strength.
        assert_allclose(anti_diagonal_result[0], 1.0, atol=1e-12)
        assert_allclose(anti_diagonal_result[3], 1.0, atol=1e-12)
        assert_allclose(anti_diagonal_result[4], -1.0, atol=1e-12)

    def test_separable_weights_have_no_temporal_trend(self):
        """A spatial profile scaled by time has zero position-time covariance."""
        ...
        # These weights factor as spatial_profile * (k + 1). The
        # conditional position distribution is identical at every time.
        assert_allclose(result[0], 0.0, atol=1e-12)
        assert_allclose(result[3:5], 0.0, atol=1e-12)
    ...
    def test_nan_weights(self):
        ...
        assert not np.isnan(result[0])  # r2
    ...
    def test_degenerate_case(self):
        ...
        result = weighted_regression_2d(weights, time_coords=time_coords)
    ...
    def test_near_zero_variance_axis_has_negligible_contribution(self):
        """Tiny orthogonal variance has a negligible effect on R-squared."""
        n = 20
        x_coords = np.arange(n, dtype=np.float64)
        # Tiny spread contributes negligible unexplained spatial variance.
    ...
    def test_normal_case_formula_regression(self):
        """Uniform posteriors have no linear temporal trend."""
        n = 10
        x_coords = np.arange(n, dtype=np.float64)
        y_coords = np.arange(n, dtype=np.float64)
        time_coords = np.arange(n, dtype=np.float64)
        ...
        expected = (cov_xt**2 + cov_yt**2) / (cov_tt * (cov_xx + cov_yy))

        assert_allclose(result[0], expected, atol=1e-12)

    def test_single_axis_reverse_has_unsigned_score_and_negative_slope(self):
        """Reversing time changes velocity, not sequence strength."""
        n = 20
        x_coords = np.arange(n, dtype=np.float64)
        y_coords = np.zeros(n, dtype=np.float64)
    ...
    def test_axis_fallback_matches_1d_weighted_correlation_with_random_weights(self):
        """With one constant spatial axis, R-squared equals squared 1D correlation."""
        ...
        assert np.isclose(result[0], corr_1d**2, rtol=1e-10, atol=1e-12)
