    ...
    def create_cross_structural_data():
        ...
        for t in assembly_times:
            ...
            for neuron in range(5, 9):  # Group 1 assembly members
                ...
                spike_times[neuron] = np.sort(
                    np.append(spike_times[neuron], jittered_time)
                )
...
def test_cross_structural_with_different_methods():
    ...
    assembly_react_ica = assembly_reactivation.AssemblyReact(
        method="ica", cross_structural=cross_structural
    )
    ...
    assembly_react_pca = assembly_reactivation.AssemblyReact(
        method="pca", cross_structural=cross_structural
    )
...
def test_cross_structural_assembly_activity():
    ...
    for t in sync_times:
        # Group 1 neurons (0-3)
        for i in range(4):
            spike_times[i] = np.sort(
                np.append(spike_times[i], t + np.random.normal(0, 0.02))
            )
        # Group 2 neurons (4-7)
        for i in range(4, 8):
            spike_times[i] = np.sort(
