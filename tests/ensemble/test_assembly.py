...
def test_cross_structural_correlation_matrix():
    ...
    corr_matrix = assembly._compute_cross_structural_correlation(zactmat, cross_structural)
    ...
    cross_group_corrs = [corr_matrix[0, 2], corr_matrix[0, 3], corr_matrix[0, 4],
                        corr_matrix[1, 2], corr_matrix[1, 3], corr_matrix[1, 4]]
...
def test_cross_structural_assemblies_synthetic():
    ...
    for t in within_group_times:
        actmat[within_group_neurons, t] += np.random.poisson(4, len(within_group_neurons))
    
    # Add cross-structural assembly
    cross_neurons_g1 = np.arange(12, 15)  # from group 1
    cross_neurons_g2 = np.arange(n_neurons_group1 + 3, n_neurons_group1 + 6)  # from group 2
    ...
    # Create group labels
    cross_structural = np.array(['Group1'] * n_neurons_group1 + ['Group2'] * n_neurons_group2)
...
def test_cross_structural_validation():
    ...
    patterns, _, _ = assembly.runPatterns(actmat, cross_structural=cross_structural_correct)
...
def test_cross_structural_no_cross_components():
    ...
    # Either no patterns detected, or patterns are not cross-structural
    if patterns is not None:
        for pattern in patterns:
            ...
            group1_active = np.sum(group1_weights > 0.1)
            group2_active = np.sum(group2_weights > 0.1)
            # May have patterns that are primarily within-group due to fallback
