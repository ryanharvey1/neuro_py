...
def test_event_triggered_average_pandas_individual():
    ...
    result_matrix, time_lags = event_triggered_average(
        timestamps, signal, events, window=[-0.5, 0.5], 
        return_average=False, return_pandas=True
    )
...
def test_event_triggered_average_edge_cases():
    ...
    # Test with no valid events
    events = np.array([15, 20])  # Outside timestamp range
    result_avg, time_lags = event_triggered_average(
        timestamps, signal, events, window=[-0.5, 0.5], return_average=True
    )
...
def test_event_triggered_average_consistency():
    ...
    result_regular, time_lags = event_triggered_average(
        timestamps_regular, signal, events, window=[-0.5, 0.5], return_average=False
    )
    ...
    result_irregular, _ = event_triggered_average(
        timestamps_irregular, signal, events, window=[-0.5, 0.5], return_average=False
    )
    
    # Results should be similar (not exactly equal due to interpolation)
    ...
    correlation = np.corrcoef(result_regular.flatten(), result_irregular.flatten())[0, 1]
...
def test_event_triggered_average_large_window():
    ...
    timestamps = np.linspace(0, 10, num=100)
    signal = np.random.randn(100, 2)
    events = np.array([1, 5, 9])  # Events near boundaries
    
    # Large window that exceeds signal bounds for some events
    result_matrix, time_lags = event_triggered_average(
        timestamps, signal, events, window=[-2, 2], return_average=False
    )
    ...
    # Some events might have NaN values due to boundary issues
...
def test_event_triggered_average_comparison_with_fast():
    ...
    result_new, time_lags_new = event_triggered_average(
        timestamps, signal, events, window=window, sampling_rate=sampling_rate,
        return_average=True
    )
    
    # Test fast function (note: signal needs to be transposed)
    result_fast, time_lags_fast = event_triggered_average_fast(
        signal.T, events, sampling_rate=sampling_rate, window=window,
        return_average=True
    )
