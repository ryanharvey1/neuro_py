...
def test_nearest_event_delay():
    ...
    assert np.array_equal(close_ts, expected_nearest), (
        "Test case 1 failed: nearest timestamps do not match"
    )
    assert np.array_equal(delay, expected_delay), (
        "Test case 1 failed: delays do not match"
    )
    assert np.array_equal(nearest_index, expected_nearest_index), (
        "Test case 1 failed: nearest indices do not match"
    )
